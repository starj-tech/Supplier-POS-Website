...
import { productsApi, transactionsApi, expensesApi } from '@/lib/api';
  ...
  const fetchProducts = useCallback(async () => {
    ...
    if (result.success && result.data) {
      ...
      if (result.error?.includes('login')) {
        ...
        toast({
          title: 'Error',
          description: result.error || 'Gagal memuat produk dari server',
          variant: 'destructive',
        });
      }
    }
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    if (result.success && result.data) {
      ...
      if (!result.error?.includes('login')) {
        toast({
          title: 'Error',
          description: result.error || 'Gagal memuat transaksi dari server',
          variant: 'destructive',
        });
      }
    }
  ...
  const fetchExpenses = useCallback(async () => {
    ...
    if (result.success && result.data) {
      ...
      if (!result.error?.includes('login')) {
        toast({
          title: 'Error',
          description: result.error || 'Gagal memuat pengeluaran dari server',
          variant: 'destructive',
        });
      }
    }
